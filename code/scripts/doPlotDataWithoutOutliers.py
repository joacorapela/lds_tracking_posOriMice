...
def main(argv):
    ...
    if n_samples < 0:
        n_samples = data.shape[0]
    times = start_time_sec + np.arange(0, n_samples) * 1.0/sample_rate
    ...
    if skip_head_orientations:
        ...
        fig = ff.create_quiver(x=implX_interpolated, y=implZ_interpolated,
                               u=(noseX_interpolated-implX_interpolated)*arrow_scale_factor,
                               v=(noseZ_interpolated-implZ_interpolated)*arrow_scale_factor,
                               showlegend=False)
    trace = go.Scatter(x=implX_interpolated, y=implZ_interpolated,
